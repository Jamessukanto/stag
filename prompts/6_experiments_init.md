...
1. A single reproducible pipeline that, from one config:
   - loads and splits the data via LibimsetiDataLoader(config, downsample=True).load() — train-negative downsampling happens here (config.negative_downsample_ratio), not inside model fit(),
   - passes the full loaded interaction list to each model's fit() (models train on what they receive; they do not re-downsample),
   - trains the MF model and the NeuMF model, writing each as a ModelArtifact to artifacts/ (record sampling_strategy on the artifact — "random" or "popularity_biased" — matching the eval distractor protocol),
   - builds a core.ground_truth.EvaluationDataset via EvaluationDataset.from_interactions(loaded_interactions, split=<eval_split>) — this filters to the eval split for ground truth AND attaches interacted_targets_by_user from the full loaded list (all splits) so eval/ can sample uninteracted ranking distractors without importing data/,
...
2. Reproducibility: one config file / CLI entry point; a single random_seed controls data splitting, training, and evaluation distractor sampling. Re-running must reproduce the same artifacts and metrics. Persist the resolved config alongside results.
...
Terminology (orchestration must respect these boundaries):
- load() + downsample=True: shapes train supervision only.
- sample_uninteracted_candidates() on DataLoader: returns users never interacted with (eval distractors). experiments/ does NOT need to call this if eval/ samples from EvaluationDataset.interacted_targets_by_user, but both must use the same pool definition and strategy semantics.
- Never downsample val/test ground truth.
...
Workflow, repeat per capability (config loading, the train step per model, EvaluationDataset construction, the evaluate sweep, result collection, end-to-end pipeline):
